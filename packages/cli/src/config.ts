...
import fs from 'fs';
import { resolveFromRoot } from '@ws-system/shared';

// Load environment variables from .env file
dotenv.config();

// Root project directory - uses enhanced path detection
const projectRoot = resolveFromRoot('.');

// Find server package if in monorepo structure
const serverPackagePath = (() => {
  const monorepoServerPath = path.join(projectRoot, 'packages/server');
  return fs.existsSync(path.join(monorepoServerPath, 'package.json'))
    ? monorepoServerPath
    : projectRoot;
})();
...
const defaults = {
  ...
  alpineTag: process.env.ALPINE_TAG || '3.20',
  nodeVersion: process.env.NODE_VERSION || '22.16.0'
};
