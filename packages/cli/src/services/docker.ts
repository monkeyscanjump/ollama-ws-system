...
import { dirs, defaults, projectRoot, serverPackagePath } from '../config';
...
const services = {
  ...
  'websocket-server': {
    build: {
      context: projectRoot,
      dockerfile: path.join(dirs.data, 'Dockerfile.generated')
    },
    image: `${defaults.containerPrefix}:${defaults.dockerTag}`,
    container_name: `${defaults.containerPrefix}-server`,
    ports: [`${defaults.port}:3000`],
    volumes: [
      // Check if we're in a monorepo and adjust the dist path accordingly
      ...(fs.existsSync(path.join(projectRoot, 'packages', 'server'))
        ? [
            // Monorepo structure - mount server package and shared package
            `${path.join(projectRoot, 'packages', 'server').replace(/\\/g, '/')}/dist:/app/packages/server/dist`,
            `${path.join(projectRoot, 'packages', 'shared').replace(/\\/g, '/')}/dist:/app/packages/shared/dist`,
            `${path.join(projectRoot, 'packages', 'server').replace(/\\/g, '/')}/node_modules:/app/packages/server/node_modules`,
            `${path.join(projectRoot, 'packages', 'shared').replace(/\\/g, '/')}/node_modules:/app/packages/shared/node_modules`,
            `${path.join(projectRoot, 'node_modules').replace(/\\/g, '/')}:/app/node_modules`,
          ]
        : [
            // Standard structure - mount dist and node_modules directly
            `${projectRoot.replace(/\\/g, '/')}/dist:/app/dist`,
            `${projectRoot.replace(/\\/g, '/')}/node_modules:/app/node_modules`
          ]
      ),
      // Common mounts for both structures
...
export function generateDockerfileContent(): string {
  ...
  const isMonorepo = projectRoot !== serverPackagePath &&
    fs.existsSync(path.join(projectRoot, 'packages', 'server'));

  // Determine the correct path to the server's index.js
  const indexPath = isMonorepo ? 'packages/server/dist/index.js' : 'dist/index.js';
